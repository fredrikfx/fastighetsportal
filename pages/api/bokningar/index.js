...
import { getBokningar, testConnection } from '../../../lib/sql';

export default async function handler(req, res) {
  if (req.method === 'GET') {
    try {
      console.log('API: Börjar hämta bokningar');
      ...
      let bokningar;
      try {
        bokningar = await getBokningar();
        console.log(`API: Hämtade ${bokningar?.length || 0} bokningar från databasen`);
      } catch (dbError) {
        console.error('API: Databasfel vid hämtning av bokningar:', dbError);
        return res.status(500).json({ 
          error: 'Databasfel',
          message: 'Kunde inte hämta bokningar från databasen',
      ...
      if (!bokningar) {
        console.error('API: Bokningar är null eller undefined efter databasanrop');
        return res.status(500).json({ 
          error: 'Datafel',
